...
def process_transparent():
    return_code = execute_program(enc_path, [
        os.path.join(script_root, "..", "images", "mirage.gif"),
        os.path.join(script_root, "..", "images", "气气.gif"),
        "-o", os.path.join(script_root, "lsb", "transparent"),
        "-t"
    ])
    if return_code != 0:
        exit(return_code)
    ...
    return_code = execute_program(dec_path, [
        os.path.join(script_root, "lsb", "transparent.gif"),
        "-o", "transparent-dec",
        "-d", os.path.join(script_root, "lsb")
    ])
    if return_code != 0:
    ...
    is_same = compare_file(
        os.path.join(script_root, "..", "images", "气气.gif"),
        os.path.join(script_root, "lsb", "transparent-dec.gif")
    )
    if not is_same:
        log_error("Failed.")
        exit(1)
